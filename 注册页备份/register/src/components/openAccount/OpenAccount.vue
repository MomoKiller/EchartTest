<template>
    <div class="content_box" @click="closeBank">
...
export default {
  ...
  methods: {
    ...
    // 切换输入方式
    switchWay() {
      this.allowShowBank = !this.allowShowBank;
      this.isShowSelect = false;
    ...
    uploadIMG(e) {
      ...
      // this.form.imgName = this.picavalue.name;
    ...
    submitForm() {
      ...
      if (this.form.email == "" && this.form.mobile == "") {
        this.$message.error("请输入手机或邮箱！");
      } else if (this.form.mobile != "" && this.form.email != "") {
        let reg = new RegExp(
          /^[a-zA-Z0-9]+([\.\-\_]?[a-zA-Z0-9]+)*@[a-zA-Z0-9%][a-zA-Z0-9%\-]*(.[a-zA-Z0-9%\/\-]+)+$/
        );
        let reg1 = new RegExp(/^1\d{6,10}$/);
        if (!reg.test(this.form.email) || !reg1.test(this.form.mobile)) {
          console.log("实名验证3");
